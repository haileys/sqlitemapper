...
impl<SqlT, T, SqlTail, TTail> FromRow<SqlTypeCons<SqlT, SqlTail>> for (T, TTail)
    where
        SqlT: SqlType,
        SqlTail: SqlTypeList,
...
{
    fn from_row<'a>(reader: RowReader<'a, SqlTypeCons<SqlT, SqlTail>>) -> Result<((T, TTail), RowReader<'a, ()>), Error> {
